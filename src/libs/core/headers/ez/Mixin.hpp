...
#define EZ_MIXIN_UNARY_OPERATOR(MixinName, op)                                    \
    template <typename Self, typename ValueType>                                  \
    struct MixinName : public MixinBase<Self, struct EZ_CONCAT(MixinName, Tag)> { \
        Self operator op() const { return Self(op this->value()); }               \
    }

#define EZ_MIXIN_BINARY_OPERATOR(MixinName, op)                                                \
    template <typename Self, typename ValueType>                                               \
    struct MixinName : public MixinBase<Self, struct EZ_CONCAT(MixinName, Tag)> {              \
        Self operator op(const Self& rhs) const { return Self(this->value() op rhs.value()); } \
...
#define EZ_MIXIN_VALUE_BINARY_OPERATOR(MixinName, op)                                             \
    template <typename Self, typename ValueType>                                                  \
    struct MixinName : public MixinBase<Self, struct EZ_CONCAT(MixinName, Tag)> {                 \
        Self operator op(ValueType&& val) const { return Self(this->value() op std::move(val)); } \
        Self operator op(const ValueType& val) const { return Self(this->value() op val); }       \
    }
